 ...
 (JNIEnv * env, jclass cls, jobjectArray ocol1, jobjectArray ocol2, jobjectArray ucol1, jobjectArray ucol2, int o_size, int u_size)
{
	...
	/*jclass objClass = (*env)->FindClass(env, "java/lang/String");
	jobjectArray re= (*env)->NewObjectArray(env,(jsize)o_size*10, objClass, 0);
	jstring jstr;
	
	int k=0;
	for(i=0;i<o_size*10;i++)
	{

			int str_len = strlen(order[i].col1)+strlen(order[i].col2)+15;
			char * outcome = (char*)malloc(sizeof(char)*str_len);
			memset(outcome, 0, sizeof(char)*str_len);
			...
			if(order[i].flag==0)
				...
				strcat(outcome,"1");
			strcat(outcome,",");
			strcat(outcome,order[i].col1);
			strcat(outcome,",");
			strcat(outcome,order[i].col2);
